...
describe("Keyboard component", () => {
  ...
  it("clicking the right key should reveal letter h in hello", async () => {
    ...
    const hiddenLetterH: HTMLSpanElement =
      await screen.findByTestId("hidden-letter-h")
    expect(hiddenLetterH.style.visibility).toBe("hidden")

    const hButton: HTMLButtonElement = screen.getByRole("button", {
      name: /h/i,
    })
    user.click(hButton)
  ...
  it("clicking the z key should reveal part of hangman and disable the z key", async () => {
    ...
    const zButton: HTMLButtonElement = screen.getByRole("button", {
      name: /z/i,
    })

    let head = screen.queryByTestId("head")
    expect(head).toBeNull()

    user.click(zButton)

    head = await screen.queryByTestId("head")
